...
VERA_RAG_KB_DESCRIPTION = (
    ...
    "Используй инструмент для правовых и иных проверяемых фактов: прав, обязанностей, "
    "запретов, гарантий, сроков, процедур, документов, последствий и органов защиты. "
    "query — самодостаточный поисковый запрос. Самодостаточное текущее сообщение "
    "передавай в query дословно, без свободной "
    "переформулировки и новых фактов. Контекст добавляй только из фактических "
    "предыдущих сообщений этой же сессии. Не добавляй инвалидность, роль или "
    "аудиторию пользователя из специализации Веры. Если сообщение состоит только "
    "из ссылки на правовую норму, передавай её символ в символ и не расшифровывай "
    "содержание. При неполной реплике без однозначного антецедента сначала уточни "
    "предмет. Уточнение приоритетнее вызова: не вызывай инструмент с дополненным "
    "догадкой query, а отдельного тематического слова недостаточно для "
    "самодостаточности. Поиск выполняется по всей доступной базе знаний без ограничения по "
    "роли пользователя. "
...
def register_vera_rag_kb(mcp: FastMCP, rag_client: RagClient, top_k: int) -> None:
    ...
    async def vera_rag_kb(
        ...
        with tracer.start_as_current_span(
            ...
            try:
                result = await rag_client.search(query=query, top_k=top_k)
