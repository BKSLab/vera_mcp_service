...
SEND_CONSULTATION_EMAIL_DESCRIPTION = (
    ...
    'письмо. Успешный результат не возвращает email: status="ok" подтверждает '
    'отправку на адрес из входного аргумента без повторного раскрытия адреса. '
    'status="error" означает, что отправка не была подтверждена. '
