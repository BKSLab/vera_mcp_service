...
> **Обновление 2026-07-26:** этот документ фиксирует историческую первую
> итерацию с `vera_rag_kb`. В сервис добавлен второй инструмент
> `send_consultation_email`, которому для узкой внутренней операции
> форматирования текста нужен LLM. Его актуальные требования, архитектура и
> статус реализации описаны в `CONSULTATION_EMAIL_TOOL_PLAN.md`.
>
> **Статус реализации:** 🔶 Итерация 1 реализована. Этапы 0–8 и 10 выполнены; прежний блокер провижининга БД `vera_rag_service` закрыт, production health-check RAG/MCP успешен 2026-07-22. Этап 9 (сквозной E2E) остаётся открытым до production-деплоя Agent Service. Дата плана: 2026-07-09, актуализирован 2026-07-22.
>
...
### Этап 5 — Observability (Phoenix) ✅ Код и тесты обновлены (2026-07-20)

MCP — ручная середина продуктового distributed trace. Автоинструментация здесь не используется.

- [x] 5.1 `observability/tracing.py` — `TracerProvider`, OTLP/HTTP exporter, `x-project-name`, `PHOENIX_ENABLED`, `force_flush` и shutdown.
- [x] 5.2 FastMCP `Context` используется для извлечения входящего W3C-контекста; `mcp.execute.vera_rag_kb` становится потомком активного Agent span.
- [x] 5.3 `RagClient` динамически inject-ит текущий context в заголовки `POST /api/v1/search`, в том числе при параллельных вызовах.
- [x] 5.4 Старый MCP-owned `rag.search` удалён: retrieval span создаёт RAG Service, которому эта операция принадлежит.
- [x] 5.5 Span не содержит query/chunk text; записываются имя инструмента, длина query, audience, outcome и технические признаки результата/ошибки.
- [x] 5.6 Lifespan закрывает общий `httpx.AsyncClient`, затем выполняет flush/shutdown tracing.

**Definition of Done на уровне кода:** известный remote parent корректно извлекается из настоящего streamable-http вызова, MCP span наследует его, а RAG-запрос получает `traceparent` от MCP span. Unit- и protocol-тесты также проверяют параллельные вызовы без смешивания контекста.

Одинаковый Phoenix endpoint/project нужен для отображения, но сам по себе не гарантирует одного дерева. Связность создаётся только W3C propagation Agent → MCP → RAG. Живое дерево трёх процессов остаётся пунктом следующей сессии в `vera_agent_service/PHOENIX_OBSERVABILITY_PLAN.md`.
...
**Definition of Done:** полный прогон `pytest tests/` зелёный локально. ✅ Актуальный набор 49/49 тестов, `ruff check .` чист (2026-07-22). CI не верифицирован реальным прогоном на GitHub Actions в этой сессии — требует проверки на первом реальном push/PR.
...
### Этап 9 — Сквозной E2E-прогон 🔶 Ожидает production-деплоя Agent Service
...
**Исторический блокер закрыт 2026-07-22:** провижининг БД исправлен через согласованные `POSTGRES_DB`/`POSTGRES_NAME`; production RAG возвращает `status=ok`, `database=ok`, а MCP — `status=ok`, `rag_service=ok`. Текущий внешний prerequisite Этапа 9 — production-деплой Agent Service.
...
**Что остаётся не проверенным:** реальный сквозной путь через RabbitMQ и SSE от лица клиента и единое дерево Agent → MCP → RAG в Phoenix. Проверяется после production-деплоя Agent Service.
...
| ~~`vera_rag_service` не провижинился до рабочего состояния~~ | ~~Блокировал Этап 9~~ | Закрыто 2026-07-22: `POSTGRES_DB` согласован, production health-check `database=ok` |
...
| ~~Agent Service ждёт этот сервис как P0-блокер~~ | ~~Блокировало первый реальный запуск~~ | Закрыто: MCP и RAG развёрнуты; остаётся production-деплой Agent Service |
...
~~1. Один общий Phoenix-инстанс на все три сервиса или три независимых~~ — **закрыто (Этап 8.3, обновлено 2026-07-20):** общий — `vera_agent_phoenix`. Agent, MCP и RAG экспортируют в один project с разными `service.name`; W3C propagation, а не общий инстанс сам по себе, связывает spans в один trace.
...
~~5. `RAG_SERVICE_API_KEY` был плейсхолдером.~~ — **закрыто:** реальное согласованное значение присутствует в production `.env` обоих сервисов; секрет не коммитится.
