...
4. **Подготовка консультации** — общий `LlmClient` получает исходный текст как данные и структурирует его без изменения темы, которую передал Agent Service. Jinja подставляет результат в фирменный HTML-бланк, WeasyPrint формирует tagged PDF/UA с Unicode-текстом. Прикладных ограничений длины текста и `max_completion_tokens` нет.
...
| Тул `send_consultation_email` | Agent Service → этот сервис | `send_consultation_email(consultation_text: str, consultation_topic: str, email: str) -> dict`. `consultation_topic` — краткая тема, из которой сервис сам формирует заголовок и русское имя PDF. Успех: `status`, `document_name`, `message`; входной email обратно не возвращается. Предусмотренная ошибка: `status="error"`, стабильные `code` и `message` |
...
| OpenAI-compatible Chat Completions | Этот сервис → LLM API | Базовый `LLM_API_URL` дополняется `/chat/completions`; structured output валидируется через `FormattedConsultation`, а тема добавляется сервисом отдельно |
