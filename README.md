...
6. **Наблюдаемость** — OpenTelemetry (без автоинструментации) → общий Arize Phoenix project. FastMCP tool извлекает входящий W3C `traceparent`, создаёт `mcp.execute.vera_rag_kb` и передаёт новый актуальный контекст в HTTP-запрос RAG. MCP не создаёт собственный `rag.search`: этот span принадлежит RAG Service.
