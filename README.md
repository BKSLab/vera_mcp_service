...
| Тул `send_consultation_email` | Agent Service → этот сервис | `send_consultation_email(consultation_text: str, consultation_topic: str, email: str) -> dict`. `consultation_topic` — краткая тема, из которой сервис сам формирует заголовок и русское имя PDF. Успех: `status`, нормализованный `email`, `document_name`, `message`. Предусмотренная ошибка: `status="error"`, стабильные `code` и `message` |
